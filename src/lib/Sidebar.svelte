<script lang="ts">
  ...
  import { deleteThread } from "../services/openaiService";
  ...
  async function deleteConversation(i: number) {
    ...
    // Wyciągnięcie konwersacji do usunięcia
    const deletedConversation = $conversations[i];
    const deletedData = await deleteThread(deletedConversation.id);
    console.log("Conversation to be deleted:", deletedData);
    ...
    if (i === $chosenConversationId) {
      ...
      console.log(
        "Selected conversation index adjusted to:",
        i > 0 ? i - 1 : 0
      );
      ...
      console.log(
        "Selected conversation index adjusted due to deletion before it. New index:",
  ...
  function exportSession() {
    ...
    a.href = url;
    a.download = "SemioGPT-session.json";
    a.click();
...
<div class="flex flex-col text-white/90">
  <div
    class="{$menuVisible == true
      ? 'translate-x-0'
      : '-translate-x-[100%] md:translate-x-0'} duration-200 h-full fixed md:flex w-[260px] flex-col bg-secondary z-40"
  >
    <nav class="flex h-full fle-1 flex-col space-y-1 p-2 bg-secondary">
      <button
        ...
        class="md:hidden z-20 flex py-3 px-3 items-center gap-3 rounded-md hover:bg-hover hover:opacity-hover transition-colors duration-200 cursor-pointer text-sm mb-2 flex-shrink-0 border border-white/50"
      >
        Close menu
      </button>
      <button
        class="flex justify-between items-center py-3 px-3 cursor-pointer w-full text-left hover:bg-gray-700 rounded-lg z-20"
        on:click={newChat}
      >
        <p class="text-center font-bold text-[1.2rem] m-0">Semio Academy AI</p>

        <img src={NewChat} alt="New chat" class="w-6 h-6 filter-white z-20" />
      </button>

      <div
        class="flex flex-col h-40 my-2 flex-grow overflow-y-auto convo-container"
      >
        ...
        {#each $conversations.slice().reverse() as conv, i}
          <div
            class="{$chosenConversationId === $conversations.length - i - 1
              ...
              : ''} title-container conversation flex justify-between min-h-[50px] py-1 pl-3 items-center rounded-md hover:bg-hover cursor-pointer text-sm transition-colors duration-200"
            tabindex="-1"
            on:click={() => {
          ...
          >
            {#if editingTitleId === $conversations.length - i - 1}
              <input
                ...
                on:blur={() => saveEditedTitle($conversations.length - i - 1)}
                on:keydown={(e) => {
                  if (e.key === "Enter") {
                    saveEditedTitle($conversations.length - i - 1);
                    e.preventDefault();
                  }
                }}
              />
            {:else}
              <p
                class="text-left text-sm flex-grow title-text {$showTokens
                  ? ''
                  : ''}"
              >
            ...
            <div class="flex items-center gap-2">
              <button
                ...
                on:click|stopPropagation={(e) => {
                  let id = $conversations.length - i - 1;
                  startEditConversationTitle(id, conv.title);
                }}
                ...
                class="edit hidden rounded w-7 h-7 font-bold flex justify-center items-center hover:bg-blue-600"
              >
                <img
                  src={EditIcon}
                  alt="Edit"
                  class="icon-white min-w-4 w-4 h-4"
                />
              </button>
              ...
              {#if $conversations.length >= 2}
                <button
                  ...
                  class="delete hidden rounded w-7 h-7 font-bold flex justify-center items-center hover:bg-warning"
                >
                  <img
                    src={CloseIcon}
                    alt="Delete"
                    class="icon-white min-w-5 w-5 h-5"
                  />
                </button>
              {/if}
            </div>
      ...
      <button
        on:click={openHelp}
        class="flex border border-white/50 py-3 px-3 items-center font-bold gap-3 rounded-md hover:bg-hover transition-colors duration-200 cursor-pointer text-sm mt-auto z-20"
      >
        Pomoc
      </button>
    </nav>
